...
public class TradeLicenseService {
    ...
    public List<TradeLicense> create(TradeLicenseRequest tradeLicenseRequest, String businessServicefromPath) {
        ...
        Object billingSlabs = util.getBillingSlabs(tradeLicenseRequest.getRequestInfo(),
                tradeLicenseRequest.getLicenses().get(0).getTenantId());
    ...
    public Map<String, Integer> countApplications(TradeLicenseSearchCriteria criteria, RequestInfo requestInfo,
            String serviceFromPath, HttpHeaders headers) {
     ...
     * @return List of tradeLicense for the given criteria
     */
    public List<TradeLicense> getLicensesWithOwnerInfo(String stateTenantId, TradeLicenseSearchCriteria criteria,
            RequestInfo requestInfo) {
    ...
    /**
     * Updates the tradeLicenses while skipping Payment
     * 
     * @param tradeLicenseRequest The update Request
     * @return Updated TradeLcienses
     */
    public List<TradeLicense> updateSkipPay(TradeLicenseRequest tradeLicenseRequest, String businessServicefromPath) {
        log.info("======Update Request For SKIP PAYMENT:{}", tradeLicenseRequest);
        TradeLicense licence = tradeLicenseRequest.getLicenses().get(0);
        TradeLicense.ApplicationTypeEnum applicationType = licence.getApplicationType();
        List<TradeLicense> licenceResponse = null;
        if (applicationType != null && (applicationType).toString().equals(TLConstants.APPLICATION_TYPE_RENEWAL) &&
            ...
            validateLatestApplicationCancellationSkipPay(tradeLicenseRequest, businessService);
    ...
    private void validateLatestApplicationCancellationSkipPay(TradeLicenseRequest tradeLicenseRequest,
            BusinessService businessService) {
        List<TradeLicense> licenses = tradeLicenseRequest.getLicenses();
        TradeLicenseSearchCriteria criteria = new TradeLicenseSearchCriteria();
        ...
        actionValidator.validateUpdateRequestSkipPay(tradeLicenseRequest, businessService, searchResultForCancellation);
    ...
    public List<TradeLicense> plainSearch(TradeLicenseSearchCriteria criteria, RequestInfo requestInfo) {
        ...
        criteria.setIds(ids);
        log.info("Before criteria {}", criteria.getTenantId());

        TradeLicenseSearchCriteria idsCriteria = TradeLicenseSearchCriteria.builder().ids(ids).build();
        idsCriteria.setTenantId(criteria.getTenantId());
        log.info("After criteria {}", idsCriteria.getTenantId());
